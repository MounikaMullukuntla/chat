...
vi.mock('@/components/document-skeleton', () => ({
  DocumentSkeleton: vi.fn(() => <div data-testid="document-skeleton">Loading...</div>),
}));
...
describe('Document Viewer (Text Artifact)', () => {
  ...
  describe('Document Display', () => {
    it('should render document content in editor', () => {
      ...
      expect(screen.getByTestId('editor-content')).toHaveTextContent('Test document content');
    });
    ...
    it('should display correct status', () => {
      ...
      expect(screen.getByTestId('editor-status')).toHaveTextContent('streaming');
    });
  ...
  describe('Edit Mode Toggle', () => {
    ...
    it('should switch to diff mode', () => {
      mockGetDocumentContentById.mockImplementation((index: number) => {
        if (index === 0) return 'Version 1 content';
        if (index === 1) return 'Version 2 content';
        return '';
      });

      const Content = textArtifact.content;
      render(<Content {...defaultProps} mode="diff" currentVersionIndex={1} />);
    ...
    it('should pass correct version content to diff view', () => {
      mockGetDocumentContentById.mockImplementation((index: number) => {
        if (index === 0) return 'Old version';
        if (index === 1) return 'New version';
        return '';
      });
  ...
  describe('Version Navigation', () => {
    describe('View Changes Action', () => {
      ...
      it('should toggle diff view on click', () => {
        const viewChangesAction = textArtifact.actions[0];
        viewChangesAction.onClick({ handleVersionChange: mockHandleVersionChange } as any);
      ...
      it('should be disabled when on version 0', () => {
        const viewChangesAction = textArtifact.actions[0];
        const isDisabled = viewChangesAction.isDisabled?.({ currentVersionIndex: 0 } as any);
      ...
      it('should be enabled when not on version 0', () => {
        const viewChangesAction = textArtifact.actions[0];
        const isDisabled = viewChangesAction.isDisabled?.({ currentVersionIndex: 1 } as any);
    ...
    describe('Previous Version Action', () => {
      ...
      it('should navigate to previous version on click', () => {
        const prevAction = textArtifact.actions[1];
        prevAction.onClick({ handleVersionChange: mockHandleVersionChange } as any);
      ...
      it('should be disabled on version 0', () => {
        const prevAction = textArtifact.actions[1];
        const isDisabled = prevAction.isDisabled?.({ currentVersionIndex: 0 } as any);
      ...
      it('should be enabled on later versions', () => {
        const prevAction = textArtifact.actions[1];
        const isDisabled = prevAction.isDisabled?.({ currentVersionIndex: 2 } as any);
    ...
    describe('Next Version Action', () => {
      ...
      it('should navigate to next version on click', () => {
        const nextAction = textArtifact.actions[2];
        nextAction.onClick({ handleVersionChange: mockHandleVersionChange } as any);
      ...
      it('should be disabled when on current version', () => {
        const nextAction = textArtifact.actions[2];
        const isDisabled = nextAction.isDisabled?.({ isCurrentVersion: true } as any);
      ...
      it('should be enabled when viewing older versions', () => {
        const nextAction = textArtifact.actions[2];
        const isDisabled = nextAction.isDisabled?.({ isCurrentVersion: false } as any);
  ...
  describe('Diff Viewer', () => {
    ...
    it('should show old and new content in diff view', () => {
      mockGetDocumentContentById.mockImplementation((index: number) => {
        if (index === 0) return 'This is the old content.';
        if (index === 1) return 'This is the new content.';
        return '';
      });
      ...
      expect(screen.getByTestId('diff-old')).toHaveTextContent('This is the old content.');
      expect(screen.getByTestId('diff-new')).toHaveTextContent('This is the new content.');
    });
    ...
    it('should compare correct version indices', () => {
      ...
      const Content = textArtifact.content;
      render(<Content {...defaultProps} mode="diff" currentVersionIndex={3} />);
  ...
  describe('Suggestion Display', () => {
    ...
    it('should render editor without suggestions when metadata has empty array', () => {
      ...
      expect(screen.getByTestId('editor')).toBeInTheDocument();
      expect(screen.queryByTestId('editor-suggestions')).not.toBeInTheDocument();
    });

    it('should pass suggestions to editor', () => {
      const Content = textArtifact.content;
      render(<Content {...defaultProps} metadata={{ suggestions: mockSuggestions }} />);

      expect(screen.getByTestId('editor-suggestions')).toBeInTheDocument();
      expect(screen.getByTestId('suggestion-sugg-1')).toHaveTextContent('old text');
      expect(screen.getByTestId('suggestion-sugg-2')).toHaveTextContent('another text');
    });

    it('should handle single suggestion', () => {
      const Content = textArtifact.content;
      render(<Content {...defaultProps} metadata={{ suggestions: [mockSuggestions[0]] }} />);
    ...
    it('should display shrink div when suggestions exist on mobile', () => {
      ...
      const { container } = render(
        <Content {...defaultProps} metadata={{ suggestions: mockSuggestions }} />
      );
  ...
  describe('Toolbar Actions', () => {
    ...
    it('should send message for final polish', () => {
      const polishAction = textArtifact.toolbar?.[0];
      polishAction?.onClick({ sendMessage: mockSendMessage, documentId: 'doc-123' } as any);
      ...
      expect(mockSendMessage).toHaveBeenCalledWith({
        ...
        parts: [
          {
            type: 'text',
            text: expect.stringContaining('Please add final polish to the document'),
          },
        ],
      });
    });
    ...
    it('should send message for suggestions', () => {
      const suggestionsAction = textArtifact.toolbar?.[1];
      suggestionsAction?.onClick({ sendMessage: mockSendMessage, documentId: 'doc-456' } as any);
