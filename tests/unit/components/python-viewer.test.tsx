...
vi.mock('@/components/python-console', () => ({
  PythonConsole: ({ output, isExecuting, onClear }: any) => (
    <div data-testid="python-console">
      ...
      {output?.stdout && <div data-testid="console-stdout">{output.stdout}</div>}
      {output?.stderr && <div data-testid="console-stderr">{output.stderr}</div>}
      {output?.error && <div data-testid="console-error">{output.error}</div>}
      {onClear && <button onClick={onClear} data-testid="console-clear-btn">Clear</button>}
    </div>
  ),
}));
...
describe('PythonViewer', () => {
  ...
  describe('Code Editor Rendering', () => {
    ...
    it('should show spinner animation when streaming with no content', () => {
      ...
      const spinner = screen.getByText('Generating code...').previousElementSibling;
    ...
    it('should display empty state when no content and not streaming', () => {
      ...
      expect(screen.getByText('Python code will appear here once generated')).toBeInTheDocument();
    });
  ...
  describe('Console Tab Rendering', () => {
    ...
    it('should auto-show console when there is stderr output', async () => {
      const { rerender } = render(<PythonViewer content="import sys; print('error', file=sys.stderr)" />);
    ...
    it('should auto-show console when there is an error', async () => {
      const { rerender } = render(<PythonViewer content="raise Exception('Test error')" />);
    ...
    it('should auto-show console when there are plots', async () => {
      const { rerender } = render(<PythonViewer content="import matplotlib.pyplot as plt" />);
    ...
    it('should pass execution output to PythonConsole component', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('console-stdout')).toHaveTextContent(output.stdout);
        expect(screen.getByTestId('console-stderr')).toHaveTextContent(output.stderr);
        expect(screen.getByTestId('console-error')).toHaveTextContent(output.error);
      });
    });
    ...
    it('should pass isExecuting state to PythonConsole', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('console-executing')).toHaveTextContent('true');
      });
    });
  ...
  describe('Execution Controls', () => {
    ...
    it('should call onExecute when Run button is clicked', async () => {
      ...
      render(<PythonViewer content="print('test')" onExecute={handleExecute} />);
  ...
  describe('Integration Tests', () => {
    ...
    it('should handle error during execution', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('console-error')).toHaveTextContent('Exception: Test');
      });
    });
  });
});
