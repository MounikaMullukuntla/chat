...
test.describe("Admin Configuration Journey", () => {
  ...
  test.describe("Provider Selection", () => {
    ...
    test("should navigate to Google provider configuration", async ({
      page,
    }) => {
      // Click on Google Configure button
      await page
        .locator("text=Google AI")
        .locator("..")
        .locator("..")
    ...
    test("should navigate to OpenAI provider configuration", async ({
      page,
    }) => {
    ...
    test("should navigate to Anthropic provider configuration", async ({
      page,
    }) => {
    ...
    test("should navigate back to dashboard from provider page", async ({
      page,
    }) => {
      // Go to Google config
  ...
  test.describe("Agent Configuration", () => {
    test.beforeEach(async ({ page }) => {
      ...
      await page.waitForSelector("text=Chat Model Agent", { timeout: 10_000 });
    });

    test("should display Chat Model Agent configuration form", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('label:has-text("System Prompt")')
      ).toBeVisible();
      await expect(
        page.locator(
          'textarea[name*="systemPrompt"], textarea[id*="systemPrompt"]'
        )
      ).toBeVisible();
    ...
    test("should update and save Chat Model Agent configuration", async ({
      page,
    }) => {
      ...
      const systemPromptField = page
        .locator('textarea[name*="systemPrompt"], textarea[id*="systemPrompt"]')
        .first();
    ...
    test("should persist configuration changes after page reload", async ({
      page,
    }) => {
      ...
      const reloadedPromptField = page
        .locator('textarea[name*="systemPrompt"], textarea[id*="systemPrompt"]')
        .first();
  ...
  test.describe("Configuration Effect Verification", () => {
    ...
    test("should verify configuration is loaded on admin dashboard", async ({
      page,
    }) => {
      ...
      await page.waitForSelector("text=Google AI", { timeout: 10_000 });
  ...
  test.describe("Error Handling", () => {
    ...
    test("should handle network errors during config load", async ({
      page,
    }) => {
  ...
  test.describe("Navigation and Breadcrumbs", () => {
    ...
    test("should display correct breadcrumbs on provider page", async ({
      page,
    }) => {
      await page.goto("/admin/google");
    ...
    test("should navigate using breadcrumb links", async ({ page }) => {
      ...
      await page
        .locator('nav[aria-label="Breadcrumb"] button:has-text("Admin")')
        .click();
