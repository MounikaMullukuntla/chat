...
const TEST_REPO_NAME = process.env.TEST_REPO_NAME || 'test-user/test-repo';
...
test.describe('GitHub MCP Journey', () => {
  ...
  test('should enter GitHub PAT and display GitHub integration UI', async ({ page }) => {
    ...
    const githubPATInput = page.locator('input[placeholder*="GitHub" i][placeholder*="PAT" i], input[placeholder*="token" i]').first();
    ...
    if (await githubPATInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      console.log('GitHub PAT input not immediately visible, may require navigation to settings');
    }
  });

  test('should connect to GitHub and load user repositories', async ({ page }) => {
    ...
    const settingsButton = page.locator('[href="/settings"], button:has-text("Settings")').first();
    ...
    const githubPATInput = page.locator('input[placeholder*="GitHub" i][placeholder*="PAT" i], input[placeholder*="Personal Access Token" i], input[type="password"]').first();

    if (await githubPATInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await githubPATInput.fill(TEST_GITHUB_PAT);

      // Look for a connect button or the UI that shows when connected
      const connectButton = page.locator('button:has-text("Connect"), button:has-text("Save")').first();
      ...
      const repoList = page.locator('[class*="repository" i], [data-testid*="repo" i]').first();
      await repoList.waitFor({ timeout: 10000 }).catch(() => {
  ...
  test('should search and select a repository', async ({ page }) => {
    ...
    const repoSearchInput = page.locator('input[placeholder*="Search repositories" i], input[placeholder*="repository" i]').first();
  ...
  test('should browse repository files in file browser', async ({ page }) => {
    ...
    if (await filesTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const fileTree = page.locator('[class*="tree" i], [class*="browser" i]').first();
      await fileTree.waitFor({ timeout: 10000 }).catch(() => {
        console.log('File tree not loaded');
      });

      // Look for folder icons that can be expanded
      const folderIcon = page.locator('svg[class*="folder" i], button:has(svg)').first();
  ...
  test('should select and load file content from repository', async ({ page }) => {
    ...
    const githubButton = page.locator('button:has-text("GitHub"), [aria-label*="GitHub" i]').first();
    ...
    if (await githubButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await fileCheckbox.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        const applyButton = page.locator('button:has-text("Apply"), button:has-text("Select"), button:has-text("Done")').first();
        ...
        if (await applyButton.isVisible({ timeout: 3000 }).catch(() => false)) {
          ...
          const selectedFileIndicator = page.locator('[class*="selected" i], [class*="context" i]').first();
  ...
  test('should ask questions about code from GitHub repository', async ({ page }) => {
    // This test verifies asking questions about GitHub code
    // Assumes GitHub context is already loaded

    // Find the message input field
    const messageInput = page.locator('textarea[placeholder*="message" i], input[placeholder*="message" i], [contenteditable="true"]').first();
    ...
    if (await messageInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await sendButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const aiResponse = page.locator('[class*="message" i][class*="assistant" i], [data-role="assistant"]').first();
        await aiResponse.waitFor({ timeout: 30000 }).catch(() => {
  ...
  test('should verify AI uses GitHub context in response', async ({ page }) => {
    ...
    const settingsLink = page.locator('[href="/settings"], button:has-text("Settings")').first();
    if (await settingsLink.isVisible({ timeout: 3000 }).catch(() => false)) {
      await settingsLink.click();
    }

    // Step 2: Enter GitHub PAT (if input is visible)
    const githubPATInput = page.locator('input[placeholder*="GitHub" i], input[placeholder*="PAT" i]').first();
    if (await githubPATInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await githubPATInput.fill(TEST_GITHUB_PAT);
    ...
    const githubContextButton = page.locator('button:has-text("GitHub"), [aria-label*="GitHub" i]').first();
    if (await githubContextButton.isVisible({ timeout: 5000 }).catch(() => false)) {
    ...
    if (await messageInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await messageInput.fill('Based on the GitHub repository, describe the main functionality');
      ...
      if (await sendButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        if (await response.isVisible({ timeout: 30000 }).catch(() => false)) {
  ...
  test('should handle GitHub PAT validation errors gracefully', async ({ page }) => {
    ...
    if (await githubPATInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await connectButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const errorMessage = page.locator('[class*="error" i], [role="alert"], [class*="danger" i]').first();
        ...
        if (await errorMessage.isVisible({ timeout: 5000 }).catch(() => false)) {
          ...
          expect(errorText?.toLowerCase()).toMatch(/invalid|error|failed|token/i);
        }
      }
    }
  });
  ...
  test('should display repository information correctly', async ({ page }) => {
    ...
    const repoElement = page.locator('[class*="repository" i], [data-testid*="repo" i]').first();

    if (await repoElement.isVisible({ timeout: 10000 }).catch(() => false)) {
      // Check for repository name
      const repoName = repoElement.locator('[class*="name" i], [class*="title" i]').first();
      await expect(repoName).toBeVisible();

      // Check for repository description (if present)
      const repoDescription = repoElement.locator('[class*="description" i]').first();
      // Description may or may not be present, so just check if element exists

      // Check for metadata like stars, forks, language
      const metadata = repoElement.locator('[class*="metadata" i], [class*="stat" i]').first();
  ...
  test('should handle file browser navigation correctly', async ({ page }) => {
    ...
    const folderIcons = page.locator('button:has(svg[class*="chevron" i]), button:has(svg[class*="folder" i])');
    ...
    if (count > 0) {
      ...
      const nestedItems = page.locator('[style*="padding-left"], [class*="nested" i]');
  ...
  test('should support multiple file selection', async ({ page }) => {
    ...
    if (checkboxCount >= 2) {
      ...
      const selectionCount = page.locator('[class*="selected" i]:has-text("2"), :has-text("2 file")').first();

      // Should show count of selected files
      if (await selectionCount.isVisible({ timeout: 3000 }).catch(() => false)) {
        await expect(selectionCount).toBeVisible();
      }
    }
  });

  test('should clear GitHub context when deselecting repositories', async ({ page }) => {
    // Test removing selected repositories

    // Find a selected repository badge or item
    const selectedRepoBadge = page.locator('[class*="badge" i], [class*="selected" i]').first();

    if (await selectedRepoBadge.isVisible({ timeout: 5000 }).catch(() => false)) {
      // Look for remove/close button on the badge
      const removeButton = selectedRepoBadge.locator('button, [role="button"]').first();
