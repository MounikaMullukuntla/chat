...
test.describe("Python Code Journey", () => {
  ...
  test("should request Python code and display it in editor", async ({
    page,
  }) => {
    // Test requesting Python code
    const messageInput = page.locator(
      'textarea[placeholder*="Message"], textarea[data-testid="message-input"], [data-testid="chat-input"]'
    );
    ...
    await page.waitForSelector(
      '[data-artifact-kind="python"], .monaco-editor',
      {
        timeout: 30_000,
        state: "visible",
      }
    ...
    const editorContent = page.locator(
      ".view-lines, .monaco-editor .view-line"
    );
    await expect(editorContent.first()).toBeVisible();
  });

  test("should display Python code with syntax highlighting", async ({
    page,
  }) => {
    // Request Python code
    const messageInput = page.locator(
    ...
    const syntaxTokens = page.locator(
      ".monaco-editor .mtk1, .monaco-editor .mtk3, .monaco-editor .mtk5"
    );
  ...
  test("should switch to console tab", async ({ page }) => {
    ...
    const consoleButton = page
      .locator('button:has-text("Show Console"), button:has-text("Console")')
      .first();

    // Wait for console button to be visible
    await consoleButton.waitFor({ state: "visible", timeout: 10_000 });
    await consoleButton.click();

    // Verify console panel is visible
    const consolePanel = page
      .locator(
        '[class*="console"], :has-text("Console"):has(div), :has-text("No output yet")'
      )
      .first();
  ...
  test("should view execution output in console", async ({ page }) => {
    ...
    const noOutputMessage = page.locator(
      'text=/No output yet|Click "Run" to execute/i'
    );
    ...
    const consoleOutput = page
      .locator('[class*="console"], pre, [class*="output"]')
      .first();
  ...
  test("should modify code and re-run successfully", async ({ page }) => {
    ...
    const isConsoleVisible = await page
      .locator("text=/No output yet|Console/i")
      .isVisible()
      .catch(() => false);
    if (!isConsoleVisible) {
      await consoleButton.click();
  ...
  test("should show loading state during code execution", async ({ page }) => {
    ...
    const loadingIndicators = page.locator(
      'text=/Running|Executing/i, button:has-text("Running")'
    );

    // Check within a short timeout
    const _hasLoadingState = await loadingIndicators
      .isVisible()
      .catch(() => false);

    // Verify run button becomes disabled during execution or shows "Running..." state
    const runButtonState = page
      .locator('button:has-text("Run"), button:has-text("Running")')
      .first();
  ...
  test("should display console toggle button", async ({ page }) => {
    ...
    const consoleVisible = page.locator(
      "text=/Console/i, text=/No output yet/i"
    );

    const hideButtonExists = await hideConsoleButton
      .isVisible()
      .catch(() => false);
    const consoleIsVisible = await consoleVisible
      .isVisible()
      .catch(() => false);
