 ...
 */
async function _waitForAIResponse(page: Page, timeout = 60_000) {
 ...
 */
async function _selectModel(page: Page, modelName: string) {
  // Click on the model selector button
  await page.click(
    'button:has-text("Gemini"), button:has-text("GPT"), button:has-text("Claude")'
  );
...
test.describe("New User Chat Session - E2E Tests", () => {
  ...
  test("Test 1: Complete first-time user flow", async ({ page }) => {
    ...
    const modelSelector = page
      .locator("button")
      .filter({ hasText: /Gemini|GPT|Claude|Loading/i })
      .first();
  ...
  test("Test 3: Model selection", async ({ page }) => {
    ...
    await page.waitForSelector(
      'button:has-text("Gemini"), button:has-text("GPT"), button:has-text("Claude"), button:has-text("Loading")',
      { state: "visible" }
    );
    ...
    const modelSelectorButton = page
      .locator("button")
      .filter({ hasText: /Gemini|GPT|Claude/i })
      .first();
  ...
  test("Test 5: Receiving response", async ({ page }) => {
    ...
    const _sendButton = page.locator(SEND_BUTTON_SELECTOR);
  ...
  test("Test 6: Multi-turn conversation", async ({ page }) => {
    ...
    const messages = page.locator(
      `text=${TEST_MESSAGE_1}, text=${TEST_MESSAGE_2}`
    );
  ...
  test("Test 7: Chat persistence after refresh", async ({ page }) => {
    ...
    try {
      ...
      console.log(
        "Note: Message may not persist if chat was not saved to database"
      );
    }
