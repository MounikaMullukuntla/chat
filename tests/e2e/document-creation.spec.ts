...
async function waitForAIResponse(page: Page) {
  ...
  await page.getByTestId("message-assistant").waitFor({ timeout: 30_000 });
...
test.describe("Document Creation Journey", () => {
  ...
  test("should complete full document creation flow", async ({ page }) => {
    ...
    await sendChatMessage(
      page,
      "Create a document about TypeScript best practices"
    );
    ...
    await expect(
      artifact.locator("text=TypeScript best practices")
    ...
    await expect(artifact.locator("text=/Updated.*ago/")).toBeVisible({
      timeout: 60_000,
    });

    // Verify some content has been streamed
    const contentArea = artifact
      .locator('[data-testid="document-content"]')
      .first();
  ...
  test("should request document creation", async ({ page }) => {
    ...
    await expect(documentToolCall).toBeVisible({ timeout: 30_000 });
  });
  ...
  test("should watch real-time streaming", async ({ page }) => {
    ...
    await sendChatMessage(
      page,
      "Create a document explaining JavaScript closures"
    );
    ...
    const _savingIndicator = artifact.locator("text=Saving changes...");

    // The artifact should be in streaming mode initially
    // Wait for the document to have some content
    await expect(artifact).toContainText("closure", { timeout: 60_000 });
  ...
  test("should show document in artifact panel", async ({ page }) => {
    ...
    await sendChatMessage(
      page,
      "Create a markdown document about Git workflow"
    );
    ...
    await expect(artifact).toContainText("Git workflow", { timeout: 30_000 });

    // Verify content area exists
    const contentArea = artifact.locator('[role="textbox"]').first();
    await expect(contentArea).toBeVisible({ timeout: 30_000 });
  });
  ...
  test("should edit document", async ({ page }) => {
    ...
    await expect(artifact.locator("text=Saving changes...")).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("should view version history", async ({ page }) => {
    ...
    const versionControls = artifact
      .locator("button")
      .filter({ hasText: /version|history/i });
  ...
  test("should compare versions", async ({ page }) => {
    ...
    await sendChatMessage(
      page,
      "Create a document explaining async/await in JavaScript"
    );
    ...
    const diffToggle = artifact
      .locator("button")
      .filter({ hasText: /diff|compare/i });
  ...
  test("should revert to previous version", async ({ page }) => {
    ...
    await page.keyboard.type(
      "## This is version 2 content that will be reverted"
    );
    ...
    const prevVersionButton = artifact
      .locator("button")
      .filter({ hasText: /previous|prev|back/i });
    ...
    if ((await prevVersionButton.count()) > 0) {
      ...
      if ((await restoreButton.count()) > 0) {
        ...
        const versionFooter = artifact.locator(
          "text=You are viewing a previous version"
        );
        ...
        await expect(artifact).not.toContainText(
          "This is version 2 content that will be reverted"
        );
      }
    }
  ...
  test("should persist document across page refresh", async ({ page }) => {
    ...
    await expect(documentMessage).toBeVisible({ timeout: 10_000 });
  });
});
...
test.describe("Document Creation - Edge Cases", () => {
  ...
  test("should handle multiple document creations in same chat", async ({
    page,
  }) => {
    ...
    await artifact1.waitFor({ state: "visible", timeout: 30_000 });
    ...
    await artifact2.waitFor({ state: "visible", timeout: 30_000 });

    // Verify it's the CSS document
    await expect(artifact2).toContainText("CSS", { timeout: 30_000 });
  });
