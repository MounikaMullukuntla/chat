...
test.describe('Mermaid Diagram Journey', () => {
  ...
  test('should request and create a Mermaid diagram', async ({ page }) => {
    ...
    const messageInput = page.locator('textarea[placeholder*="message"], textarea[placeholder*="Message"]').first();
    await messageInput.fill('Create a flowchart showing the user authentication process with login, verification, and success/error states');
    ...
    const artifactPanel = page.locator('[data-testid="artifact-panel"], .artifact, [class*="artifact"]').first();
    await expect(artifactPanel).toBeVisible({ timeout: 10000 });
  });

  test('should display streaming animation during diagram generation', async ({ page }) => {
    ...
    try {
      ...
      console.log('Streaming indicator not caught (may have completed too fast)');
    }
  });
  ...
  test('should render Mermaid diagram in live preview', async ({ page }) => {
    ...
    const messageInput = page.locator('textarea[placeholder*="message"], textarea[placeholder*="Message"]').first();
    await messageInput.fill('Create a sequence diagram showing client-server communication');
    ...
    await expect(svgDiagram).toBeVisible({ timeout: 15000 });
  ...
  test('should allow editing diagram code', async ({ page }) => {
    ...
    const editButton = page.locator('button:has-text("Edit"), button:has-text("Code"), [role="tab"]:has-text("Code")').first();

    if (await editButton.isVisible({ timeout: 5000 })) {
      await editButton.click();

      // Find the code editor (textarea or Monaco editor)
      const codeEditor = page.locator('textarea[class*="code"], textarea[class*="editor"], .monaco-editor textarea').first();
  ...
  test('should update preview when diagram code is edited', async ({ page }) => {
    ...
    const codeTab = page.locator('[role="tab"]:has-text("Code"), button:has-text("Code")').first();
    if (await codeTab.isVisible({ timeout: 5000 })) {
      ...
      if (await codeEditor.isVisible({ timeout: 3000 })) {
        ...
        const previewTab = page.locator('[role="tab"]:has-text("Preview"), button:has-text("Preview")').first();
      ...
      if (await codeEditor.isVisible({ timeout: 3000 })) {
        ...
        if (await previewTab.isVisible({ timeout: 3000 })) {
          ...
          try {
            ...
            const isRendered = await svg.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should support exporting Mermaid diagram', async ({ page }) => {
    ...
    if (await exportButton.isVisible({ timeout: 5000 })) {
      ...
      try {
        ...
        console.log('Download not triggered in expected way - may use different export mechanism');
      }
    } else {
  ...
  test('should handle diagram version control', async ({ page }) => {
    ...
    if (await codeTab.isVisible({ timeout: 5000 })) {
      ...
      if (await codeEditor.isVisible({ timeout: 3000 })) {
        // Modify the diagram
        await codeEditor.fill('stateDiagram-v2\n    [*] --> Active\n    Active --> Inactive\n    Inactive --> [*]');
        ...
        const versionIndicator = page.locator(
          'text=/v\\d+/, [class*="version"], text="Version"'
        ).first();
        ...
        if (await versionHistory.isVisible({ timeout: 3000 })) {
          ...
          const versionList = page.locator('[role="list"], ul, [class*="version-list"]').first();
  ...
  test('should support different Mermaid diagram types', async ({ page }) => {
    ...
    for (const { request, expectedKeyword } of diagramTypes) {
      ...
      const isVisible = await svg.isVisible({ timeout: 10000 }).catch(() => false);
  ...
  test('should display diagram with zoom and pan controls', async ({ page }) => {
    // Setup and create diagram
    const apiKeyInput = page.locator('input[placeholder*="API"]').first();
    if (await apiKeyInput.isVisible()) {
      await apiKeyInput.fill('test-api-key-for-e2e-testing');
    ...
    const zoomIn = page.locator('button:has-text("+"), button[title*="zoom in"], button[aria-label*="zoom in"]').first();
    const zoomOut = page.locator('button:has-text("-"), button[title*="zoom out"], button[aria-label*="zoom out"]').first();
...
test.describe('Mermaid Diagram Edge Cases', () => {
  ...
  test('should handle very large diagram code', async ({ page }) => {
    ...
    const hasContent = await page.locator('svg, [class*="loading"], text="Generating"').first().isVisible({ timeout: 10000 }).catch(() => false);
