...
import { useAuthContext, useRequireAuth as contextRequireAuth, useRequireAdmin as contextRequireAdmin } from './context'
 ...
 * }
 */
export function useRequireAdmin(redirectTo: string = '/'): UseRequireAuthReturn {
  const result = contextRequireAdmin(redirectTo)
  
  // Convert to the expected return type format
  if (result.loading || !result.user || !result.session || result.role !== 'admin') {
    return {
      user: {} as User, // Placeholder to satisfy TypeScript
      session: {} as Session, // Placeholder to satisfy TypeScript
...
export function useAuthState() {
  const { user, role, loading, isAuthenticated, isAdmin, isUser } = useAuthContext()
