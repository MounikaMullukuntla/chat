...
export function createVerificationService(provider: 'google' | 'anthropic' | 'openai') {
  switch (provider) {
    case 'google':
      const { GoogleVerificationService } = require('./google-verification-service');
      return new GoogleVerificationService();
    case 'anthropic':
      const { AnthropicVerificationService } = require('./anthropic-verification-service');
      return new AnthropicVerificationService();
    case 'openai':
      const { OpenAIVerificationService } = require('./openai-verification-service');
...
export function createGitHubVerificationService() {
  const { GitHubVerificationService } = require('./github-verification-service');
