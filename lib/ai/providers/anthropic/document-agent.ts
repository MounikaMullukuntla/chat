...
/**
 * Anthropic Document Agent implementation
 * Handles document creation and editing operations
export class AnthropicDocumentAgent {
  ...
  async *chat(params: ChatParams): AsyncGenerator<StreamingResponse, void, unknown> {
    try {
      ...
      console.error('Anthropic Document Agent error:', error);
  ...
  private validateConfig(): void {
    ...
    if (!this.config.enabled) {
      throw new AgentError(
        ...
        'Anthropic document agent is disabled'
      );
    }
    ...
    if (!this.config.availableModels || this.config.availableModels.length === 0) {
      throw new AgentError(
        'anthropic-document',
        ErrorCodes.INVALID_CONFIGURATION,
        'No models configured for Anthropic document agent'
      );
    }
    ...
    if (!this.config.tools) {
      throw new AgentError(
        'anthropic-document',
        ErrorCodes.INVALID_CONFIGURATION,
        'No tools configured for Anthropic document agent'
      );
    }
  }
