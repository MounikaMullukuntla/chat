 ...
 * Anthropic Python Agent implementation
 * Handles Python code generation and artifact creation
export class AnthropicPythonAgent {
  ...
  async *chat(params: ChatParams): AsyncGenerator<StreamingResponse, void, unknown> {
    try {
      ...
      console.error('Anthropic Python Agent error:', error);
  ...
  private validateConfig(): void {
    ...
    if (!this.config.enabled) {
      throw new AgentError(
        ...
        'Anthropic Python agent is disabled'
      );
    }
    ...
    if (!this.config.availableModels || this.config.availableModels.length === 0) {
      throw new AgentError(
        'anthropic-python',
        ErrorCodes.INVALID_CONFIGURATION,
        'No models configured for Anthropic Python agent'
      );
    }
    ...
    if (!this.config.tools) {
      throw new AgentError(
        'anthropic-python',
        ErrorCodes.INVALID_CONFIGURATION,
        'No tools configured for Anthropic Python agent'
      );
    }
  }
