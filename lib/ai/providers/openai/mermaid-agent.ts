 ...
 * OpenAI Mermaid Agent implementation
 * Handles Mermaid diagram creation and editing
export class OpenAIMermaidAgent {
  ...
  async *chat(params: ChatParams): AsyncGenerator<StreamingResponse, void, unknown> {
    try {
      ...
      console.error('OpenAI Mermaid Agent error:', error);
  ...
  private validateConfig(): void {
    ...
    if (!this.config.enabled) {
      throw new AgentError(
        ...
        'OpenAI Mermaid agent is disabled'
      );
    }
    ...
    if (!this.config.availableModels || this.config.availableModels.length === 0) {
      throw new AgentError(
        'openai-mermaid',
        ErrorCodes.INVALID_CONFIGURATION,
        'No models configured for OpenAI Mermaid agent'
      );
    }
    ...
    if (!this.config.tools) {
      throw new AgentError(
        'openai-mermaid',
        ErrorCodes.INVALID_CONFIGURATION,
        'No tools configured for OpenAI Mermaid agent'
      );
    }
  }
