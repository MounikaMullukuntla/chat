...
export class OpenAIProviderToolsAgent {
  ...
  async *chat(params: ChatParams): AsyncGenerator<StreamingResponse, void, unknown> {
    try {
      ...
      console.error('OpenAI Provider Tools Agent error:', error);
  ...
  private getAvailableTools() {
    ...
    if (this.config.tools.codeExecution?.enabled) {
      tools.codeExecution = tool({
        ...
        execute: async ({ code, language, timeout }: { code: string; language: string; timeout: number }) => {
          try {
  ...
  private validateConfig(): void {
    ...
    if (!this.config.enabled) {
      throw new AgentError(
        ...
        'OpenAI provider tools agent is disabled'
      );
    }
    ...
    if (!this.config.availableModels || this.config.availableModels.length === 0) {
      throw new AgentError(
        'openai-provider-tools',
        ErrorCodes.INVALID_CONFIGURATION,
        'No models configured for OpenAI provider tools agent'
      );
    }
    ...
    if (!this.config.tools) {
      throw new AgentError(
        'openai-provider-tools',
        ErrorCodes.INVALID_CONFIGURATION,
        'No tools configured for OpenAI provider tools agent'
      );
    }
  }
