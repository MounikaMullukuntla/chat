...
export class OpenAIGitMcpAgent {
  ...
  async *chat(params: ChatParams): AsyncGenerator<StreamingResponse, void, unknown> {
    try {
      ...
      console.error('OpenAI Git MCP Agent error:', error);
  ...
  private validateConfig(): void {
    ...
    if (!this.config.enabled) {
      throw new AgentError(
        ...
        'OpenAI Git MCP agent is disabled'
      );
    }
    ...
    if (!this.config.tools || Object.keys(this.config.tools).length === 0) {
      throw new AgentError(
        'openai-git-mcp',
        ErrorCodes.INVALID_CONFIGURATION,
        'No tools configured for OpenAI Git MCP agent'
      );
    }
  }
