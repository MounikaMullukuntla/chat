...
export async function GET(
  ...
  try {
    ...
    if (!model) {
      return new ChatSDKError("not_found:api", `Model ${modelId} not found`).toResponse();
    }
    ...
    return new ChatSDKError("bad_request:database", "Failed to retrieve model").toResponse();
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      return new ChatSDKError("bad_request:api", `Validation failed: ${error.errors.map(e => e.message).join(', ')}`).toResponse();
    }
  ...
  try {
    ...
    return new ChatSDKError("bad_request:database", "Failed to update model").toResponse();
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return new ChatSDKError("bad_request:database", "Failed to delete model").toResponse();
  }
}