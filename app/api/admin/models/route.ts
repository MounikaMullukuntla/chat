...
const CreateModelSchema = z.object({
  ...
  outputPricingPerMillionTokens: z.string().min(1, "Output pricing is required"),
  ...
  try {
    ...
    if (provider) {
      if (!['google', 'openai', 'anthropic'].includes(provider)) {
        return new ChatSDKError("bad_request:api", `Invalid provider: ${provider}`).toResponse();
      }
    ...
    return new ChatSDKError("bad_request:database", "Failed to retrieve models").toResponse();
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return new ChatSDKError("bad_request:database", "Failed to create model").toResponse();
  }
}